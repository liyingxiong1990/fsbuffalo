...
import com.buffalo.gateway.order.service.DelivererOrderService;
...
public class DelivererOrderController {
	...
	public <T> Map<String, T> get3Day(@RequestParam(value = "pageNum",defaultValue = "1")Integer pageNum, @RequestParam(value = "pageSize",defaultValue = "10")Integer pageSize,
		...
		try {
			PageHelper.startPage(pageNum,pageSize);
			List<DelivererOrder> list = delivererOrderService.get3Day(keyword);
			PageInfo<DelivererOrder> pageInfo = new PageInfo<DelivererOrder>(list);
			return  (Map<String, T>) ResponseUtil.result(HttpStatus.OK, "查询三天内送货单列表成功", pageInfo);
		} catch (Exception e) {
			e.printStackTrace();
			// TODO Auto-generated catch block
			return  (Map<String, T>) ResponseUtil.result(HttpStatus.INTERNAL_SERVER_ERROR, "查询三天内送货单列表失败. " + e.getMessage());
		}
	}
