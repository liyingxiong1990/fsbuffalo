<template>
  <div class="enterprise-product">
    <base-table tableHandLeftPlaceholder="名称\规格\排序" :pageQuery="table.pageQuery" :addTableRowClick="addAlertOpen" :rowContextdblClick="getAlertOpen" :tableCols="table.cols" :contextMenuData="table.contextMenuData"></base-table>
    <product-dialog :dialog="dialog" :submitCallback="submitCallback"></product-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    deleteAlertOpen (row) {
      ...
      this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
        ...
        this.$store.state.http.auto('product', 'delete', { data: row }).then((response) => {
