      ...
      vm.$refs[form].validate((valid) => {
        if (valid) {
          ...
          if (vm.dialog.type === 'post') {
            successMessage = '缴仓新增成功！'
            errorMessage = '缴仓新增失败！'
            requestMethod = 'add'
          }
          if (vm.dialog.type === 'put') {
            successMessage = '缴仓修改成功！'
            errorMessage = '缴仓修改失败！'
            requestMethod = 'update'
          }
          this.$store.state.http.auto('carrier', requestMethod, { data: this.dialog.data }).then((res) => {
