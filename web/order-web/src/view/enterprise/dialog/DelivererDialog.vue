    ...
    <el-dialog :title="dialog.title" :visible.sync="dialog.visible" width="380px" @open="dialogOpen" :before-close="dialogClose">
      <el-form label-width="120px" :model="dialog.data"  :class="dialog.type === 'get'?'form-get':''" label-position="right" :rules="dialog.rules" ref="ruleForm">
        <el-form-item label="姓名" prop='name'>
          <el-input v-model="dialog.data.deliverer_name" :disabled="dialog.type === 'get'" clearable size="mini"></el-input>
        </el-form-item>
        <el-form-item label="路线" prop='delivery_lines'>
          <el-input v-model="dialog.data.delivery_lines" :disabled="dialog.type === 'get'" clearable size="mini"></el-input>
        </el-form-item>
      ...
      vm.$refs[form].validate((valid) => {
        if (valid) {
          ...
          if (vm.dialog.type === 'post') {
            successMessage = '外县市司机新增成功！'
            errorMessage = '外县市司机新增失败！'
            requestMethod = 'add'
          }
          if (vm.dialog.type === 'put') {
            successMessage = '外县市司机修改成功！'
            errorMessage = '外县市司机修改失败！'
            requestMethod = 'update'
          }
          this.$store.state.http.auto('deliverer', requestMethod, { data: this.dialog.data }).then((res) => {
