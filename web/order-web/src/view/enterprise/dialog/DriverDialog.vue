      ...
      vm.$refs[form].validate((valid) => {
        if (valid) {
          ...
          if (vm.dialog.type === 'post') {
            successMessage = '专卖店司机新增成功！'
            errorMessage = '专卖店司机新增失败！'
            requestMethod = 'add'
          }
          if (vm.dialog.type === 'put') {
            successMessage = '专卖店司机修改成功！'
            errorMessage = '专卖店司机修改失败！'
            requestMethod = 'update'
          }
          this.$store.state.http.auto('driver', requestMethod, { data: this.dialog.data }).then((res) => {
