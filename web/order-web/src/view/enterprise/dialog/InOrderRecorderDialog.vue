<template>
  <div class="inOrderRecorder-dialog">
      ...
      vm.$refs[form].validate((valid) => {
        if (valid) {
          ...
          if (vm.dialog.type === 'post') {
            successMessage = '仓管新增成功！'
            errorMessage = '仓管新增失败！'
            requestMethod = 'add'
          }
          if (vm.dialog.type === 'put') {
            successMessage = '仓管修改成功！'
            errorMessage = '仓管修改失败！'
            requestMethod = 'update'
          }
          this.$store.state.http.auto('inOrderRecorder', requestMethod, { data: this.dialog.data }).then((res) => {
