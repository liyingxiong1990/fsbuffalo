<template>
  <div class="outOrderRecorder-dialog">
      ...
      vm.$refs[form].validate((valid) => {
        if (valid) {
          ...
          if (vm.dialog.type === 'post') {
            successMessage = '开单人新增成功！'
            errorMessage = '开单人新增失败！'
            requestMethod = 'add'
          }
          if (vm.dialog.type === 'put') {
            successMessage = '开单人修改成功！'
            errorMessage = '开单人修改失败！'
            requestMethod = 'update'
          }
          this.$store.state.http.auto('outOrderRecorder', requestMethod, { data: this.dialog.data }).then((res) => {
