...
export default {
  ...
  created () {
    this.initData()
    this.$store.state.http.auto('sysPermission', 'getAll').then(res => {
  ...
  methods: {
    initData () {
      ...
      this.$store.state.http.get('/sys/menu', 'getAll').then(res => {
        ...
        // 格式化成树形结构
        this.treeData = this.formatTreeData(menuData, 'SYS')
        // 菜单排序
        let sortFn = (data1) => {
          data1.sort((a, b) => {
      ...
      data.children.map(obj => {
        ...
        if (obj.children.length) {
          ...
          for (let i = 0; i < this.permissionData.length; i++) {
            // 判断menu_id是否为当前点击的checkbox的子菜单menu_id
